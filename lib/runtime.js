...
import { runLogQuery } from "./query.js";
import { resolveConfig } from "./config.js";
/** 剥离 ANSI 转义与控制字符（与 vendored lq.py 的 _clean 语义一致）。 */
...
export function registerTool(ctx, rawConfig) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                lines.push(`级别统计：ERROR ${stats.errors} / WARN ${stats.warns} / INFO ${stats.infos} / DEBUG ${stats.debugs}${stats.others > 0 ? ` / OTHER ${stats.others}` : ''}`);
                ...
                if (r.filter.lines.length > 0) {
                    ...
                    for (const line of r.filter.lines) {
                        // 清洗控制字符/ANSI 转义，防止日志内容注入终端或误导模型
                        const raw = stripControl(line.raw);
                        lines.push(`[${line.lineno}]${line.timestamp ? ` ${line.timestamp}` : ''} ${raw}`);
                    }
                }
                if (r.analysis) {
                    lines.push('--- 归因 ---');
                    lines.push(r.analysis.summary);
                    for (const e of r.analysis.evidence)
                        lines.push(`证据：${e}`);
                    for (const s of r.analysis.suggestions)
                        lines.push(`建议：${s}`);
                }
